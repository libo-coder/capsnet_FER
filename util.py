...
class CapsLayer(Layer):
    def __init__(self, num_output=32, batch_size=128, length_dim=8, num_caps=None,
                 layer_type="pcap", num_rout_iter=3, **kwargs):
    ...
    def routing(self, input):
        ...
        # weight shape (32, 32, 16, 16, 7, 8, 16)
        print(input.shape)
        input_shape = input.shape.as_list()
        weight_shape = [input_shape[1], self.num_caps, input_shape[4], self.length_dim]

        self.W = self.add_weight(shape=weight_shape, initializer='glorot_uniform', name='W')

        self.b_IJ = self.add_weight(shape=[1, input_shape[1], self.num_caps,1,1],
                                    initializer="zeros", name='bias', trainable=False)

        inputs_hat = tf.scan(lambda ac, x: K.batch_dot(x, self.W, [3, 2]),
                             elems=input, initializer=K.zeros([input_shape[1], self.num_caps, 1, self.length_dim]))

        print("uhat ", inputs_hat.shape)

        for iter in range(self.num_rout_iter):
            # b_IJ shape b_J (1, 8192, 7, 1, 1)
    ...
    def squash(self, vector):
        ...
        scalar_factor = vec_squared_norm / (0.5 + vec_squared_norm) / K.sqrt(vec_squared_norm)
...
class CapsNet(object):
    ...
    def train(self):
        ...
        self.x_train, self.y_train = self.load_dataset(
            "/Users/libo/Documents/Deep_Learning/CapsNet_Emotion_Recognition/fer2013/train",True)

        self.x_test, self.y_test = self.load_dataset(
            "/Users/libo/Documents/Deep_Learning/CapsNet_Emotion_Recognition/fer2013/test",True)
        ...
        self.model.fit(x_train, y_train, validation_data=(x_test,y_test), epochs=10, batch_size=128)
        ...
        self.model.fit_generator(datagen.flow(x_train, y_train, batch_size=128),
        ...
        self.model.save_weights("/Users/libo/Documents/Deep_Learning/CapsNet_Emotion_Recognition/models/fer2013-model.h5")
